...
class Go4LunchFactory: ViewModelProvider.Factory {
    ...
    init {
      ...
      //  application = MainApplication.getApplication()!!
        ...
        this. workMatesChoiceRepository = WorkMatesChoiceRepository(Clock.systemDefaultZone())
        this. notificationsRepository = NotificationsRepository(context)
        this. getNearbySearchResultsUseCase = GetNearbySearchResultsUseCase(locationRepository, nearbySearchRepository, application)
        this. getNearbySearchResultsByIdUseCase = GetNearbySearchResultsByIdUseCase(locationRepository, nearbySearchRepository, application)
        this.  getRestaurantDetailsResultsUseCase = GetRestaurantDetailsResultsUseCase(locationRepository, nearbySearchRepository, restaurantDetailsRepository, application)
        this.  getRestaurantDetailsResultsByIdUseCase = GetRestaurantDetailsResultsByIdUseCase(restaurantDetailsRepository)
        this. getPredictionsUseCase = GetPredictionsUseCase(locationRepository, autocompleteRepository)
        this. getCurrentUserIdUseCase = GetCurrentUserIdUseCase()
        this.   clickOnChoseRestaurantButtonUseCase = ClickOnChoseRestaurantButtonUseCase(firebaseFirestore, firebaseAuth, Clock.systemDefaultZone())
        this.   clickOnFavoriteRestaurantUseCase = ClickOnFavoriteRestaurantUseCase(firebaseFirestore)
    }
